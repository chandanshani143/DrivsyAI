...
export async function getAdminTestDrives({ search = "", status = "" }) {
  try {
    ...
    // Add status filter
    if (status) {
      where.status = status;   //status from params passed
    }
...
export async function getDashboardData() {
  try {
    ...
    const completedTestDriveCarIds = testDrives
      .filter((td) => td.status === "COMPLETED").map((td) => td.carId);
      .map((td) => td.carId);


      const soldCarsAfterTestDrive = cars.filter(
      (car) =>car.status === "SOLD" && completedTestDriveCarIds.includes(car.id)).length;
