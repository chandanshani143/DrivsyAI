...
export async function saveWorkingHours(workingHours) {
  try {
    ...
    // Update working hours - first delete existing hours before inserting new ones
...
export async function updateUserRole(userId, role) {
  try {
    const { userId: adminId } = await auth();    //renaming to adminId to avoid conflict
    if (!adminId) throw new Error("Unauthorized");

    // Check if user is admin
    const adminUser = await db.user.findUnique({
      where: { clerkUserId: adminId },    //checking clearkUserId = adminId in the db
    });
    ...
    await db.user.update({
      where: { id: userId },
      data: { role },        //shorthand for data: { role: role }
    });
